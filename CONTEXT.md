...
```mermaid
graph TD
    A[Electron Main Process: main-electron.cjs] --> B[Browser Renderer Window]
    B --> C[React App.tsx]
    ...
    C --> J[Autosave storage synchronizer]
```
...
The application features local retro game audio cues. Instead of loading heavy static audio files, the audio system utilizes the browser's native **Web Audio API**:
...
### 3. State Management
Unlike the Coaster app which uses `Zustand` stores, the Labyrinth project manages its interface states directly inside `App.tsx` and delegates history/caching to two modular React hooks:
...
- **`useLabyrinthStorage.ts`**: Handles local saves and auto-saves to prevent layout configuration loss between launches.

---

## 📁 Key File Mappings

- **[src/main-electron.cjs](src/main-electron.cjs)**: Window manager. Polls local host ports `3000`/`3001` before displaying the app window.
