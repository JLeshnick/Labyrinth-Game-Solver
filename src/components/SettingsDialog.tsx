...
import {
  Settings, Palette, Volume2, VolumeX, Cpu, RefreshCw, Keyboard,
} from "lucide-react";
...
export function SettingsDialog({
  ...
  showToast,
}: Omit<SettingsDialogProps, 'settingsTab' | 'setSettingsTab' | 'saveName' | 'setSaveName' | 'allSlots' | 'peekSlotKey' | 'setPeekSlotKey' | 'peekedState' | 'onSaveSlot' | 'onLoadSlot' | 'onDeleteSlot'>) {
  const [settingsTab, setSettingsTab] = useState<"preferences" | "appearance" | "application">("preferences");
  ...
  return (
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      ...
      <DialogContent className="sm:max-w-[90vw] w-[95vw] sm:w-[90vw] h-[90vh] max-h-[90vh] app-dialog-panel border-stone-800 text-stone-100 shadow-2xl p-0 rounded-2xl flex flex-col overflow-hidden" onKeyDown={(e) => {
        if (e.key === " ") {
          e.stopPropagation();
        }
      }}>
        ...
        <div className="flex-1 flex flex-col md:flex-row min-h-0 overflow-hidden">
          ...
          <div className="w-full md:w-56 border-b md:border-b-0 md:border-r border-stone-800/80 flex flex-row md:flex-col py-2 px-3 md:py-4 md:px-3 gap-1.5 md:gap-1 overflow-x-auto md:overflow-x-visible shrink-0 bg-stone-950/50 scrollbar-none">
            <div role="tablist" aria-label="Settings sections" className="flex flex-row md:flex-col gap-1 sm:gap-1.5 min-w-max md:w-full">
            {sidebarTabs.map((tab) => (
              <button
                key={tab.key}
                role="tab"
